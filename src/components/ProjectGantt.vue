...
function toggleHide(projectName: string) {
  const newSet = new Set(hiddenProjects.value)
  if (newSet.has(projectName)) {
    newSet.delete(projectName)
  } else {
    newSet.add(projectName)
  }
  hiddenProjects.value = newSet
}
...
<template>
  <div class="project-gantt">
    <!-- 已隱藏項目 Chip Bar -->
    <div v-if="hiddenProjects.size > 0" class="hidden-items-bar">
      <span class="hidden-label">已隱藏:</span>
      <button 
        v-for="name in hiddenProjects" 
        :key="name" 
        class="hidden-chip"
    ...
    <div class="gantt-container">
      ...
      <div
        ...
        v-show="!hiddenProjects.has(project.projectName)"
        class="project-group"
        ...
        <div class="project-group-inner">
          ...
          >
            <button 
              ...
              @click="toggleHide(project.projectName)"
              title="隱藏此專案"
            >
